...
from .models import (
    Category, FormationType, Formation, Curriculum, FormationReview,
    OnlineFormation, OnlineFormationModule, Lesson, OnlineFormationEnrollment, LessonProgress,
    Quiz, QuizQuestion, QuizAnswer, QuizAttempt, QuizResponse,
    Certificate, CertificateVerification, LiveStream, LiveStreamAttendance, LiveStreamMessage,
    UserInteraction, ContentRecommendation, UserPreference
)
...
class OnlineFormationAdmin(admin.ModelAdmin):
    list_display = ['title', 'coach', 'category', 'access_type', 'credit_cost', 'total_enrolled', 'is_published']
...
class LessonAdmin(admin.ModelAdmin):
    list_display = ['module', 'title', 'order', 'lesson_type', 'is_preview', 'is_downloadable']
...
class OnlineFormationEnrollmentAdmin(admin.ModelAdmin):
    list_display = ['user', 'online_formation', 'progress_percentage', 'completed', 'payment_date']
...
class LessonProgressAdmin(admin.ModelAdmin):
    list_display = ['enrollment', 'lesson', 'started', 'completed', 'marked_as_studied', 'completion_percentage']
...
class QuizAdmin(admin.ModelAdmin):
    list_display = ['title', 'creator', 'visibility', 'passing_score', 'is_published', 'total_attempts', 'average_score']
    list_filter = ['visibility', 'is_published', 'randomize_questions', 'randomize_answers']
...
class QuizAttemptAdmin(admin.ModelAdmin):
    list_display = ['user', 'quiz', 'attempt_number', 'percentage', 'passed', 'started_at', 'completed_at']
    list_filter = ['passed']
...
class CertificateAdmin(admin.ModelAdmin):
    list_display = ['title', 'user', 'certificate_type', 'verification_code', 'is_public', 'issued_at']
...
class CertificateVerificationAdmin(admin.ModelAdmin):
    list_display = ['certificate', 'verified_by_ip', 'verifier_organization', 'verified_at']
    list_filter = ['verified_at']
    search_fields = ['certificate__verification_code', 'verifier_email', 'verifier_organization']
...
class LiveStreamAdmin(admin.ModelAdmin):
    list_display = ['title', 'host', 'status', 'scheduled_start', 'current_viewers', 'peak_viewers']
...
class LiveStreamAttendanceAdmin(admin.ModelAdmin):
    list_display = ['user', 'stream', 'joined_at', 'watch_duration_minutes', 'eligible_for_certificate']
...
class LiveStreamMessageAdmin(admin.ModelAdmin):
    list_display = ['user', 'stream', 'message_type', 'is_pinned', 'is_answered', 'created_at']
...
class UserInteractionAdmin(admin.ModelAdmin):
    list_display = ['user', 'interaction_type', 'formation', 'online_formation', 'created_at']
...
class ContentRecommendationAdmin(admin.ModelAdmin):
    list_display = ['user', 'reason', 'confidence_score', 'shown', 'clicked', 'enrolled', 'created_at']
...
class UserPreferenceAdmin(admin.ModelAdmin):
    list_display = ['user', 'preferred_level', 'preferred_delivery', 'preferred_language', 'recommendation_frequency']
    list_filter = ['preferred_level', 'preferred_delivery', 'preferred_language', 'recommendation_frequency']
    search_fields = ['user__username']
