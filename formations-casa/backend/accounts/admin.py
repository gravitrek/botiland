...
from .models import (
    User, CoachProfile, TrainingCenterProfile,
    VerificationRequest, PlatformSettings, CreditTransaction
)


@admin.register(User)
class UserAdmin(BaseUserAdmin):
    list_display = ['username', 'email', 'role', 'is_verified', 'is_approved', 'credits', 'total_points', 'level']
    ...
    fieldsets = BaseUserAdmin.fieldsets + (
        ...
        ('Verification', {
            'fields': ('is_verified', 'verification_requested', 'verification_requested_at', 'verified_at', 'verified_by')
        }),
        ('Approval (Legacy)', {
...
class VerificationRequestAdmin(admin.ModelAdmin):
    list_display = ['user', 'status', 'credits_paid', 'created_at', 'reviewed_at', 'reviewed_by']
    ...
    fieldsets = (
        ('User Information', {
            'fields': ('user', 'credits_paid', 'created_at')
        }),
        ('Request Details', {
            'fields': ('motivation', 'credentials', 'website', 'social_proof')
        }),
        ('Documents', {
            'fields': ('id_document', 'credential_document')
        }),
        ('Review', {
...
class PlatformSettingsAdmin(admin.ModelAdmin):
    list_display = ['credit_price_mad', 'verification_badge_cost', 'default_platform_percentage', 'updated_at']

    fieldsets = (
        ('Credit Pricing', {
            'fields': ('credit_price_mad',)
        }),
        ('Feature Costs', {
            'fields': ('verification_badge_cost', 'featured_formation_cost', 'premium_listing_cost')
        }),
        ('Revenue Sharing', {
            'fields': ('default_platform_percentage',)
        }),
        ('Credit Packages', {
            'fields': (
...
class CreditTransactionAdmin(admin.ModelAdmin):
    list_display = ['user', 'transaction_type', 'amount', 'balance_after', 'payment_status', 'created_at']
    ...
    fieldsets = (
        ...
        ('Related Objects', {
            'fields': ('online_formation', 'verification_request')
        }),
        ('Payment Details', {
